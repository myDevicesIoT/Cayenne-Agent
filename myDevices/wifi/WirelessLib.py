...
def cmd(cmd):
    ...
    try:
        ...
        #    cmd, shell=True,
        #    stdout=subprocess.PIPE, stderr=subprocess.STDOUT).stdout.read().decode('ascii')
...
class Wireless:
    ...
    def __init__(self, interface=None):
        ...
        self._driver_name = Wireless._detectDriver()
    ...
    def interfaces():
        driver_name = Wireless._detectDriver()
        if driver_name == 'nmcli':
            return NmcliWireless.interfaces()
        elif driver_name == 'wpa_supplicant':
            return WpasupplicantWireless.interfaces()
        elif driver_name == 'networksetup':
            return NetworksetupWireless.interfaces()
...
class WpasupplicantWireless(WirelessDriver):
    _file_sufix = '/etc/wpa_supplicant/'
    _file_prefix = 'wpa_supplicant.conf'
    _file = None#'/etc/wpa_supplicant/wpa_supplicant.conf'
    _interface = None

    # init
    def __init__(self, interface=None):
        self.interface(interface)
        self._file = self._file_sufix + self._file_prefix
        if interface != None:
            self._file = self._file_sufix + interface + self._file_prefix
    # connect to a network
    def connect(self, ssid, password):
        ...
        # after doing corner cases 4 seconds is a small timefram -> up to 10
        sleep(10)
